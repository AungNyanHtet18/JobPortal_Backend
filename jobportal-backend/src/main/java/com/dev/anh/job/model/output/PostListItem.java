...
public record PostListItem(
	...
	public static void select(Join<Post, Account> account, CriteriaQuery<PostListItem> cq, CriteriaBuilder cb, Root<Post> root, String username) {
		...
		reactionJoin.on(
			cb.equal(
			  reactionJoin.get(PostReact_.reactPost), true));
		...
		if(username != null && !username.isEmpty()) {
			...
			userReactionJoin.on(
			    cb.and(
		            cb.isTrue(userReactionJoin.get(PostReact_.reactPost)),
		            cb.equal(
		                userReactionJoin.get(PostReact_.account).get(Account_.email), username
			    )));
